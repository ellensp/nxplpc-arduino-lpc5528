...
framework_version = str(int(''.join(['{0:0=3d}'.format(int(x)) for x in platform.get_package_version('framework-arduino-lpc5528').split('.')]))) + 'UL'
...
#        UPLOADCMD='"$PYTHONEXE" "$UPLOADER" $UPLOADERFLAGS $SOURCE'
        UPLOADCMD='pyocd flash -t LPC55S28 $SOURCE'
    )
    upload_actions = [
        env.VerboseAction("$UPLOADCMD", "Uploading $SOURCE")
    ]
